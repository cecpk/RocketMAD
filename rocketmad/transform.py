...
def transform_from_wgs_to_gcj(latitude, longitude):
    if is_location_out_of_china(latitude, longitude):
        ...
        adjust_lat = ((adjust_lat * 180.0)
                      / ((a * (1 - ee)) / (magic * math.sqrt_magic) * pi))
        adjust_lon = ((adjust_lon * 180.0)
                      / (a / math.sqrt_magic * math.cos(rad_lat) * pi))
        adjust_lat += latitude
...
def is_location_out_of_china(latitude, longitude):
    if (longitude < 72.004 or longitude > 137.8347
            or latitude < 0.8293 or latitude > 55.8271):
        return True
    return False
