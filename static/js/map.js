...
function initSettingVariables() {
    ...
    includedRaidLevels = showConfig.raids ? Store.get('includedRaidLevels') : []
...
function initSidebar() {
    ...
    $('#raid-levels-select').on('change', function (e) {
        includedRaidLevels = $('#raid-levels-select').val().map(Number)
        reprocessGyms()
        lastgyms = false
        updateMap()
        Store.set('includedRaidLevels', includedRaidLevels)
    })
    ...
    $('#egg-levels-select').val(includedEggLevels)
    $('#raid-levels-select').val(includedRaidLevels)
...
function isGymMeetsRaidFilters(gym) {
    if (showRaids && isValidRaid(gym.raid)) {
        ...
        if (isUpcomingRaid(raid)) {
            ...
            if (!includedRaidLevels.includes(raid.level) || (raid.pokemon_id &&
